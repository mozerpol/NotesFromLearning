...
A PLL is characterized by the frequency range, jitter, jitter attenuation and lock time. The jitter of the VCO and so the jitter of the output frequency is due to different mechanisms. The lock time of a PLL is defined as the time that elapses (pol. *upływa*) from initial or reset condition till to the correct output frequency. <br/>
Nowdays PLLs are mixed-mode circuitries, that must combine analog parts as loop filter and VCO with digital parts as the frequency divider. So analog circuitry must be adapted to every new CMOS process technology and it's very difficult, because is hard to reduce the size of the loop filter, because a capacitor of significant size is needed.
